"""
Понижающий оператор (свёртка) - вид вычислений при котором применяется некоторый
оператор для некоторого множества элементов в результате которого получается
скалярное значение.

Свойства:
    - Данный оператор способен сводить некий массив элементов к одному с
    калярному значению;
    - Получаемый конечный результат (определённое скалярное значение) должен
     быть получен путём создания и исчисления частичных задач.
...
class ReductionConsumer(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            print('Running process {} on numbers {} and {}'.format(
                pname, num1, num2,
            ))
            self.task_queue.task_done()
...
def reduce_sum(array):
    ...
    while result_size > 1:
        ...
        # Не используем qsize(), т.к. он ненадёжный...
